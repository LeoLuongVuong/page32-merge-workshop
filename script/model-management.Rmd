...
```{r}
# Check for parent model diagnostics and open if present, for comparison
mod_to_compare <- PARENT_MODEL # change this to compare to other models

path_to_compare <- glue::glue("{MOD_DIR}/{mod_to_compare}/diagnostics-quick-{mod_to_compare}.html")
if (file.exists(path_to_compare)) {
  browseURL(path_to_compare)
} else {
  message(glue::glue("No diagnostics output found at {path_to_compare}"))
}
```
