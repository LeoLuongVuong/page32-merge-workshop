...
source(here("script/functions/bbr-helpers.R")) # check this out
...
# keep the log/record of what models that you've run so far
```
...
```{r, eval=FALSE}
PARENT_MODEL <- 101 # Name of parent model to copy from (typically, most recently completed model)
THIS_MODEL <- 102   # Name of new model to create (based on PARENT_MODEL)
...
# ctrl + shift + c shortcut: comment/uncomment
```
...
model_diff(mod)
# model diff - can pass and compare two arbitrary models
```
...
mod <- add_description(mod, "Copy model 101")
...
mod <- mod %>% add_notes("Model 101 copied")
...
# View(df)
# merge input with output datasets
```
...
model_diagnostics(
  ...
  browseURL()
# "here" package is a great one to point to the directory.
```
...
mod <- add_notes(mod, "I noticed CWRES changed")
...
modelSpecifics <- list(
  run_mrggsave = TRUE,  # switch to TRUE to write out individual PNGs for each figure
  # very minimal ggsave .png function
...
model_diagnostics(
  ...
  browseURL()
# change the .yml file, from estimated GFR to eGFR
```
